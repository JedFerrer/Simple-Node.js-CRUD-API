...
var BookCreateForm = require('../../forms/book-create-form');
...
var bookController = {
  ...
  store: function(req, res, next) {
    ...
    // sample book form validation
    // to check if the values are not empty
    var bookCreateForm = BookCreateForm(inputs);
    bookCreateForm.save(function(err, result) {
      if (err) {
        return res.status(422).json(err);
      }

      if (! result) {
        return res.status(422).json(bookCreateForm.formErrors);
      }
